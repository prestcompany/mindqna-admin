...
**§5의 판단 기준(필터 조회 1초)을 3배로 넘긴다.** 그리고 코드의 `MAX_EXECUTION_TIME(10000)`이 10초인데 콜드 상태의 9.3초는 그 문턱에 붙어 있고, `≥ 1`은 확실히 넘는다. 이 측정이 아래 개정을 불렀다.

### 채택 (개정) — 필터 없는 임계값은 히스토그램, 나머지는 파생 테이블

```
필터가 있는가?  아니면 cardCount 아닌 버킷이 섞였는가?
  아니오 → 히스토그램 1회 통과 후 산술 (10분 캐시)
  예     → 버킷마다 파생 테이블 + STRAIGHT_JOIN
```

히스토그램은 `Card` 인덱스를 한 번 훑어 `카드 수 → 공간 수` 표를 만든다. 모든 임계값이 이 표의 산술이다.
...
`< N`에 전체 공간 수가 필요한 이유는 **카드가 하나도 없는 공간이 히스토그램에 안 잡히기 때문**이다. prod 기준 297,614개 중 2,700개가 그렇다. 이걸 빼먹으면 `질문 1개 미만`이 0으로 나온다.

prod 실측: 구축 **7,075ms**, 이후 캐시 적중 동안 산술만. 버킷별 경로는 20과 50 둘만 물어도 매 요청 4,819ms다.

**결정적인 건 속도가 아니라 답할 수 있느냐다.** 버킷별 SQL은 `≥ 1`, `< 1`, `= 7`을 10초 가드 안에 끝내지 못해 **시간 초과 행을 돌려준다.** 히스토그램은 같은 표에서 즉답한다.

필터가 붙으면 히스토그램이 못 쓴다 — 공간을 이미 접어버려서 그중 어느 것이 ko인지 말할 수 없다. 그 경로는 파생 테이블 그대로이고 prod에서 3,237ms다.

### 두 경로가 같은 답을 내는지

살아있는 DB에서 두 번 재면 구축과 조회 사이에 데이터가 움직여 1씩 어긋난다. **단일 `REPEATABLE READ` 트랜잭션 안에서 재면 9개 연산자 전부 일치한다.** 어긋남이 결함이 아니라 시점 차이임을 이렇게 확인했다.
