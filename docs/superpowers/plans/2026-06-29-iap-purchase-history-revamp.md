...
```ts
  it('filters by status=success without the dedup lookup', async () => {
    ...
    // success는 dedup 조회 없이 단일 findMany(메인) 호출
    expect(prisma.purchaseMeta.findMany).toHaveBeenCalledTimes(1);
    expect(prisma.purchaseMeta.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ isSuccess: true }) }),
    );
  });

  it('filters by status=failed using the not-in dedup set', async () => {
    ...
    await service.getPurchaseMetas({ page: 1, status: 'failed' });
  ...
  it('applies platform and isProduction to the base where', async () => {
    ...
    await service.getPurchaseMetas({ page: 1, status: 'success', platform: 'IOS', isProduction: false });
...
Run: `cd /Users/gargoyle92/Documents/backend/mindqna-server && npx jest src/admin/product/product.service.spec.ts -t "status=success|status=failed|platform and isProduction"`
Expected: FAIL (현재 구현은 status/platform/isProduction 무시 → where에 미반영, 또는 success에서 findMany 2회 호출).

- [ ] **Step 4: 서비스 구현**

`src/admin/product/product.service.ts`의 `getPurchaseMetas`를 아래로 교체한다(기존 메서드 본문 전체 대체):
```ts
  async getPurchaseMetas(
    ...
    if (status === 'success') {
      effectiveWhere = { ...baseWhere, isSuccess: true };
...
Expected: PASS (기존 dedup 테스트 + 신규 3개).
...
const PLATFORM_META: Record<string, { variant: 'softNeutral' | 'softInfo'; text: string }> = {
  IOS: { variant: 'softInfo', text: 'iOS' },
  AOS: { variant: 'softNeutral', text: 'Android' },
  EVENT: { variant: 'softNeutral', text: 'EVENT' },
};
  ...
  const columns: ColumnDef<PurchaseMeta>[] = [
    {
      ...
      header: '번호',
      size: 70,
      cell: ({ row }) => <span className='text-sm tabular-nums text-slate-500'>{row.original.id}</span>,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center gap-1'>
            ...
            <Button variant='ghost' size='sm' className='h-7 w-7 p-0' onClick={() => copyToClipboard(userId, '유저 ID')}>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center gap-1'>
            ...
            <Button variant='ghost' size='sm' className='h-7 w-7 p-0' onClick={() => copyToClipboard(value, '상품 ID')}>
              <Copy className='h-3.5 w-3.5' />
            </Button>
          </div>
        );
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center gap-1'>
            ...
            <Button variant='ghost' size='sm' className='h-7 w-7 p-0' onClick={() => copyToClipboard(value, '결제 ID')}>
              <Copy className='h-3.5 w-3.5' />
            </Button>
          </div>
        );
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex items-center gap-1'>
            {log ? (
              <Button variant='ghost' size='sm' className='h-7 px-2 text-xs text-slate-600' onClick={() => showDetail(log, '로그 상세')}>
      ...
      <DefaultTableBtn className='justify-between'>
        ...
        >
          <div className='space-y-2'>
            <Label className='text-xs text-slate-500'>유저 ID</Label>
            <Input
          ...
          <div className='space-y-2'>
            <Label className='text-xs text-slate-500'>환경</Label>
