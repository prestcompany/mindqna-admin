...
- [x] **Step 3: 대상 수가 조건을 따라 움직이는지 확인**
...
- [x] **Step 4: 예약으로 저장한다 — 즉시 발송을 쓰지 않는다**
...
- [x] **Step 8: 발견한 문제를 고치고 커밋**

Step 2~6에서 문제가 나오면 고치고, 재현 가능한 것은 테스트를 함께 추가한 뒤 커밋한다. 문제가 없으면 커밋 없이 다음으로.

**Task 4 결과 (2026-08-27):** 통과 — 다만 통과하기까지 결함 넷을 고쳐야 했다. 이 태스크가 존재하는 이유가 여기에 다 있다.

*검증된 것* (dev DB, 조건 `커플`, 8,014명)

| 항목 | 결과 |
|---|---|
| 저장 | 201, 5행, `groupId` 1종, 905ms |
| 행별 인원 | 2000·2000·2000·2000·**14** — 꼬리 보존 |
| 합계 | 8,014 = 레일 숫자와 일치 |
| target | 전부 `USER` |
| 목록 접기 | 한 줄, `조건 · 8,014명`, 배지 `0/5` |
| 그룹 취소 | 한 번의 취소로 5행 **전부 `CANCELED`**, 남은 예약 0 |

*드러난 결함 넷*

1. **패널 안에서 날짜를 고를 수 없었다** (어드민 `b22bdea`). 모달 Sheet가 `body`에 `pointer-events: none`을 걸고 자기 콘텐츠에만 되살리는데, Popover는 포털로 그 형제가 되어 잠금을 상속했다. 달력은 보이지만 클릭이 뒤 요소로 통과한다 — **예약 발송을 UI로 만들 수 없었다.** 대시보드에서는 `body`가 잠기지 않아 멀쩡했고 그래서 아무도 못 봤다. 푸시·유저·쿠폰·공간·PDF 패널이 전부 해당됐다.
2. **청크 2,000이 수기 입력 상한 1,000에 걸려 캠페인이 전부 저장 불가였다** (서버 `add2493`). `createFilteredPush`가 청크마다 `buildData`를 부르고, 거기서 `MAX_USER_NAMES`를 검사한다. 서버가 만든 청크를 서버가 거부했다. Task 7에서 상한을 "수기용"이라 판단해 그대로 둔 것이 원인이다.
3. **예상 소요가 실제의 1/4이었다.** 레일 4분, 확인 다이얼로그 1분. 두 곳이 서로 다른 말을 했고 둘 다 대기열을 세지 않았다.
4. **접힌 캠페인의 대상 열이 첫 청크만 읽었다.** 같은 줄에 `개인 · 2000명`과 `0 / 8,014`가 나란히 있었다.

*그 밖에 관찰된 것 (이번 범위 밖)*

- 목록 하단 `총 N건`이 `totalPage × 10`으로 계산된다. 서버가 총 개수를 주지 않아서인데, 8건일 때 `총 10건 중 1-10`으로 표시된다. 접기가 행 수를 줄이면 더 어긋난다.
- 로컬 Next 개발 서버가 두 번 `main.js`/`_app.js`를 503으로 내주고 매니페스트를 잃었다. `.next` 삭제 후 재시작으로 해결.
...
- [x] **Step 4: 문제가 있으면 고치고 커밋**

```bash
rm -f probe.ts
```

**Task 5 결과 (2026-08-27):** 부분 통과.

- **매칭 0명** — 펫 레벨 9999로 저장 시도 → `400 {"code":"PUSH_FILTER_NO_MATCH"}`. 행 생성 없음. 통과.
- **상한 초과** — **dev 데이터로는 재현 불가.** 가장 넓은 조건(조건 없음, 전체 로케일)이 47,687명이라 상한 50,000에 못 미친다. `PUSH_FILTER_TOO_MANY` 경로는 단위 테스트로만 덮여 있고 실물로는 확인하지 못했다. 확인하려면 상한을 임시로 낮추거나 프로덕션 규모의 데이터가 필요하다.
- 확인 중 실수로 46,047명짜리 캠페인 24행을 만들었고, 그룹 취소로 24행 전부 `CANCELED` 처리했다 — 의도치 않았지만 그룹 취소가 24행 규모에서도 동작한다는 증거가 됐다.
...
**Task 6 결과 (2026-08-27):** 통과 — 그리고 이 태스크가 "범위 밖 한계"로 적어 두려던 문제를 실제로 고쳤다.

사용자가 목록을 보고 지적했다: 페이지와 번호가 이상하다. 원인은 **접기는 클라이언트에서, 자르기는 서버에서 행 단위로** 일어난 것이었다. 5행 캠페인이 한 페이지의 10칸 중 5칸을 먹고 한 줄로 보였고, 하단은 `총 20건 중 1-10`인데 6줄만 있었으며, 경계에 걸친 캠페인은 두 페이지에 각각 나타났다.

`COALESCE(groupId, id)`로 묶어 **캠페인 단위로 페이지를 자르도록** 서버를 고쳤다(서버 `2dc9873`, 어드민 `097c1a1`).

| 확인 | 결과 |
|---|---|
| 1페이지 | 원본 38행 → **10개 항목** |
| 2페이지 | 1행 → 1개 항목 |
| 총계 | `총 11건` (항목 수, 행 수 아님) |
| 두 페이지에 겹치는 캠페인 | **0건** |
| 번호 | 1~10 연속 (기존: 10, 24, 15, 8…) |
...
- [x] **Step 2: 예약으로 저장하고 크론이 잡을 시각까지 기다린다**
...
- [x] **Step 4: 목록 배지가 진행을 따라가는지 본다**

`0/N` → `1/N` → … → `N/N`. 마지막 행이 끝나기 전에 `N/N`이 되면 `finishedParts` 집계가 틀린 것이다.

- [x] **Step 5: 실제 기기에서 알림을 확인한다**

푸시 재작업 전체의 미결 항목이 여기서 함께 닫힌다. 제목·내용·딥링크가 의도대로 보이는지 본다.

- [x] **Step 6: 임시로 바꾼 것을 되돌리고 커밋**

Step 1에서 청크를 낮췄다면 되돌린다. 발견한 문제는 회귀 테스트와 함께 커밋한다.

**Task 8 결과 (2026-08-27):** 실기기 전송을 제외하고 통과.

로컬에서 크론을 켜려면 `NODE_ENV=production`이어야 하고, 그러면 `.env.production`이 로드되어 **프로덕션 Firebase 자격증명으로 dev 사용자 토큰에 발송**하게 된다. 실패한 토큰을 죽은 것으로 보고 `fcmToken`을 지울 수 있어 하지 않았다.

대신 `runAdminPushTick`을 **dev DB에 대고 직접 돌리되 FCM만 스텁으로** 바꿨다. 행 선점·순서·커서·상태 전이는 전부 실제 코드가 실제 데이터에 대고 실행된다.

조건 `커플+ko+질문10` = 2,400명 → 2행(#15 2000명, #16 400명), 예약 시각 도달 후:

```
틱 전  : #15 SCHEDULED 0/2000  |  #16 SCHEDULED 0/400
틱 1 후: #15 SENT 2000/2000    |  #16 SCHEDULED 0/400
틱 2 후: #15 SENT 2000/2000    |  #16 SENT 400/400
FCM 호출 5회, 토큰 2400개
```

**한 틱에 한 행씩, 순서대로.** 이것이 Task 7의 유예창 계산이 딛고 선 전제였고, 이제 측정됐다. FCM 멀티캐스트 5회는 2000÷500=4, 400÷500=1로 정확하다. 목록도 `2/2 · 2,400/2,400 · 100% · 발송 완료`로 반영됐다.

**남은 것은 실기기 알림 확인 하나뿐이다.** dev 배치 서버가 떠야 하고, 그것은 배포 문제다.
...
- [x] **Step 3: dev DB에 남은 테스트 데이터가 없는지 최종 확인**
...
- [x] **Step 5: 운영 반영 전 체크리스트를 사용자에게 전달**
