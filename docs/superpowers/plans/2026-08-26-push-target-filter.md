...
저장 후 확인:
- 토스트가 `17개로 나뉘어 등록되었습니다` 형태로 뜬다(숫자는 조건에 따라 다름).
- 목록에 **한 줄만** 늘어난다.
- 제목 옆에 `0/17` 배지가 있다.
...
Expected: 모든 행이 같은 `groupId`, `target='USER'`, 마지막을 뺀 각 행이 정확히 1000명, 합계가 레일이 보여준 숫자와 일치.
...
캠페인의 행들이 페이지 경계에 걸치면 접기가 페이지마다 쪼개진다. 목록은 서버 페이지네이션(`PAGE_SIZE = 10`)이므로 10행을 넘는 캠페인은 두 페이지에 나뉜다.

10행 이상 나오는 조건으로 캠페인을 하나 더 만들고, 1페이지와 2페이지에 **같은 캠페인이 각각 한 줄씩** 나타나는지 확인한다.

Expected(현재 구현): 두 줄로 보인다. 이는 알려진 한계다.
...
```markdown
**알려진 한계 — 페이지 경계.** 목록은 서버에서 10행씩 페이지네이션하므로 10행을 넘는
캠페인은 두 페이지에 걸쳐 각각 한 줄로 나타난다. 접기는 페이지 안에서만 일어난다.
50행짜리 캠페인이면 5페이지에 다섯 줄이다. 서버가 그룹 단위로 페이지네이션하도록
바꾸면 해결되지만, 이번 범위 밖이다.
```
...
```bash
git add docs/superpowers/specs/2026-08-26-push-target-filter-design.md
git commit -m "docs(push): record the list folding page-boundary limit"
```
