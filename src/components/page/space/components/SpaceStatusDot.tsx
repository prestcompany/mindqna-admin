...
function SpaceStatusDot({ active, className }: SpaceStatusDotProps) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 text-xs font-medium', className)}>
      <span
        className={cn('h-1.5 w-1.5 rounded-full', isUnknown ? 'bg-faint' : active ? 'bg-success' : 'bg-slate-400')}
        aria-hidden
      />
      <span className={isUnknown ? 'text-muted-foreground' : active ? 'text-success' : 'text-muted-foreground'}>
        {label}
      </span>
    </span>
  );
