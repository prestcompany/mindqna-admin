        ...
        <Card className='min-w-0 shadow-none'>
          ...
          <CardContent className='space-y-3'>
            <div className='flex flex-col gap-3 rounded-lg border bg-muted/20 px-3 py-3 sm:flex-row sm:items-center sm:justify-between'>
              ...
              <Button type='button' variant='outline' size='icon' onClick={() => copyId(detail.id)} className='h-8 w-8 shrink-0'>
        ...
        <CardContent className='space-y-3'>
          {detail.profiles?.length ? (
            detail.profiles.map((profile) => {
              ...
              return (
                <div key={profile.id} className='flex flex-col gap-3 rounded-lg border px-3 py-3 sm:flex-row sm:items-start sm:justify-between'>
                  ...
                  <Button type='button' variant='ghost' size='sm' className='w-fit' onClick={() => copyId(profile.user?.username ?? profile.id)}>
                    복사
                  </Button>
                </div>
              );
                  ...
                  <div key={meta.id}>
                    <div className='flex flex-col gap-2 py-3 sm:flex-row sm:items-start sm:justify-between'>
                      <div className='min-w-0 flex-1 space-y-2'>
                        <div className='flex flex-wrap items-center gap-2'>
                          <Badge variant={meta.amount < 0 || meta.isUse ? 'warning' : 'success'}>{buildCoinMetaLabel(meta)}</Badge>
