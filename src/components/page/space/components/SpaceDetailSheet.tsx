                ...
                <CardContent className='space-y-3'>
                  <div className='flex items-center justify-between gap-3 rounded-lg border bg-muted/20 px-3 py-2'>
                    <div className='min-w-0'>
                      ...
                      <div className='truncate font-mono text-sm'>{detail.id}</div>
                    </div>
                    <Button
                      type='button'
                      variant='outline'
                  ...
                  <div className='grid grid-cols-2 gap-3 text-sm'>
                    <div className='rounded-lg border px-3 py-2'>
                      ...
                      <div className='mt-1 font-medium'>{formatDate(detail.createdAt)}</div>
                    </div>
                    <div className='rounded-lg border px-3 py-2'>
                      <div className='text-xs text-muted-foreground'>마지막 카드 발급</div>
                      <div className='mt-1 font-medium'>{formatDate(detail.latestCardIssuedAt)}</div>
                    </div>
                    <div className='rounded-lg border px-3 py-2'>
                      <div className='text-xs text-muted-foreground'>다음 카드 생성 기준</div>
                      <div className='mt-1 font-medium'>{detail.cardGenDate ?? '-'}</div>
                    </div>
                    <div className='rounded-lg border px-3 py-2'>
                      <div className='text-xs text-muted-foreground'>삭제 예정일</div>
                      <div className='mt-1 font-medium'>{formatDate(detail.dueRemovedAt)}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent className='space-y-3'>
                {detail.profiles?.length ? (
                  detail.profiles.map((profile) => {
                    ...
                    return (
                      <div key={profile.id} className='flex items-start justify-between gap-3 rounded-lg border px-3 py-3'>
                        <div className='min-w-0 space-y-1'>
                          ...
                          <div className='text-sm text-muted-foreground'>{profile.user?.username ?? '-'}</div>
                        </div>
                        <Button type='button' variant='ghost' size='sm' onClick={() => copyId(profile.user?.username ?? profile.id)}>
                          복사
                        </Button>
                      </div>
                    );
                          ...
                          <div key={meta.id}>
                            <div className='flex items-start justify-between gap-3 py-3'>
                              <div className='min-w-0 space-y-1'>
                                ...
                                <div className='text-sm text-muted-foreground'>{meta.description || '사유 없음'}</div>
                              </div>
