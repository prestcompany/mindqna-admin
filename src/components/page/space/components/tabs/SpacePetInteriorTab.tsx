...
function Thumb({ uri, alt }: { uri?: string | null; alt: string }) {
  if (!uri) {
    return (
      <div className='flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-muted text-faint'>
  ...
  return <img src={uri} alt={alt} className='h-12 w-12 shrink-0 rounded-lg border border-border object-cover' />;
}
  ...
  return (
    <div className='space-y-6'>
      ...
      <section className='space-y-2'>
        <h3 className='text-base font-semibold text-foreground'>펫</h3>
        <div className='rounded-lg border border-border bg-white p-4'>
          {data.pet ? (
            <div className='flex flex-wrap items-center gap-x-3 gap-y-1'>
              <span className='font-medium text-foreground'>{getPetTypeLabel(data.pet.type) ?? '-'}</span>
              <span className='tabular-nums text-muted-foreground'>Lv.{data.pet.level}</span>
              <span className='tabular-nums text-muted-foreground'>EXP {data.pet.exp.toFixed(1)}</span>
            </div>
          ) : (
            <span className='text-sm text-muted-foreground'>펫 정보가 없습니다.</span>
          )}
          {data.customs.length ? (
      ...
      {/* 인테리어 보유 목록 */}
      <section className='space-y-2'>
        <h3 className='text-base font-semibold text-foreground'>인테리어 {data.interiorItems.length}</h3>
              ...
              >
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  <div className='mt-0.5 flex flex-wrap items-center gap-1'>
                    {item.template?.type ? (
                      <Badge variant='softNeutral'>
                        {INTERIOR_TYPE_LABEL[item.template.type] ?? item.template.type}
                      </Badge>
                    ) : null}
                    {item.template?.category ? (
                      <span className='text-xs text-muted-foreground'>{item.template.category}</span>
                    ) : null}
                  </div>
                </div>
              </div>
      ...
      {/* 방 */}
      <section className='space-y-2'>
        <h3 className='text-base font-semibold text-foreground'>방 {data.rooms.length}</h3>
        {data.rooms.length ? (
